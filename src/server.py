...
def _speak_with_stop_listener(voice_out, text: str):
    """Speak text while a background thread listens for a stop command."""
        ...
        while not stop_flag.is_set():
            try:
                if detect_stop_command():
                    logger.info("[Voice] Stop command — interrupting speech.")
                    if hasattr(voice_out, "stop_speech"):
                        voice_out.stop_speech()
                    break
            except Exception as e:
                logger.warning(f"[Voice] Stop listener error: {e}")
                break
...
async def execute_action(action: dict):
    ...
    try:
        if atype == "OPEN_APP":
            ...
            result = await asyncio.to_thread(run_shell, content.strip())
...
async def run_agent(user_text: str, on_working=None) -> str:
    ...
    for loop in range(MAX_TOOL_LOOPS):
        # Run synchronous chat() in a thread so the event loop stays responsive
        result = await asyncio.to_thread(chat, messages, all_tools, decision.tier, decision.model)
        content = result["content"]
        tool_calls = result.get("tool_calls", [])

        # Log actual cost computed from real token counts
        memory.log_usage(
        ...
        # Text-format tool call fallback (local models output prose instead of JSON)
        if not tool_calls and content:
            ...
            if text_calls:
                ...
                for call in text_calls:
                    res = await asyncio.to_thread(dispatch_tool, call["name"], call["args"], skill_fns)
                    messages.append({
                        "role": "user",
...
async def speak_and_broadcast(text: str, next_state: str = "idle"):
            ...
            if msg_type == "ping":
                ...
                try:
                    async def _on_working(ack_text: str):
                        ...
                        await speak_and_broadcast(spoken, next_state="working")
...
# Alias for newer clients that connect to /ws
@app.websocket("/ws")
async def ws_alias(websocket: WebSocket):
    await ws_voice(websocket)
